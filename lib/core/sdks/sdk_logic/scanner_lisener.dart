...
class _CodeScanListenerState extends State<CodeScanListener> {
  ...
  bool _keyBoardCallback(KeyEvent keyEvent) {
    bool isArabicKey =
        ArabicEnglishNumbers.isArabicNum(keyEvent.logicalKey.keyLabel, 1);
    String result =
        ArabicEnglishNumbers.convertDigitsLang(keyEvent.logicalKey.keyLabel, 1);
    switch ((keyEvent, widget.useKeyDownEvent)) {
      case (KeyEvent(logicalKey: final key), _)
          when key.keyId > 255 && key != suffixKey && !isArabicKey:
        return false;
      ...
      case (final KeyUpEvent event, false):
        _controller.sink.add(isArabicKey ? result : event.logicalKey.keyLabel);
        return false;
...
class CustomKey extends KeyEvent {
  const CustomKey({
...
class ArabicEnglishNumbers {
//   0 for english to arabic
//   1 for arabic to english
  static String convertDigitsLang(String input, int mode) {
    ...
    String replacment(String match) => mode == 1
        ? arabicNums.indexOf(match).toString()
        : arabicNums[int.parse(match)];
    for (var match in matches) {
      input = input.replaceAll(match, replacment(match));
    }
    return input;
  }

  static bool isArabicNum(String input, int mode) {
