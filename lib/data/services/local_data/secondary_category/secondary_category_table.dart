...
class SecondaryCategoryTable {
  SecondaryCategoryTable._();

  static String name = 'secondary_categories';
  ...
  static Future<List<SecondaryCategoryModel>> getAll() async {
    var list = await AppDB.read(table: name);
    return list.map((e) => SecondaryCategoryModel.fromMap(e)).toList();
  }

  static Future<SecondaryCategoryModel?> getById(int? id) async {
    var list =
        await AppDB.read(table: name, where: 'tableId = ?', whereArgs: [id]);
    return list.isNotEmpty ? SecondaryCategoryModel.fromMap(list.first) : null;
  }

  static Future<List<SecondaryCategoryModel>> getByPrimaryCategoryId(
  ...
  static Future<int> insert(SecondaryCategoryModel item) async {
    return await AppDB.insert(table: name, values: item.toMap());
  }

  static Future<int> update(
      {required SecondaryCategoryModel item, required int id}) async {
  ...
  static Future<void> insertAndUpdateIfExist(
      SecondaryCategoryModel item) async {
